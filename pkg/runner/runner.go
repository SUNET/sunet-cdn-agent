...
func (agt *agent) reloadHAProxy(containerName string) error {
	...
	stdout, stderr, err := agentutils.RunCommand("docker", "kill", "--signal", "USR2", containerName)
	if err != nil {
...
func (agt *agent) setupNftables(cnc cdntypes.CacheNodeConfig, nftablesConfDir string) error {
	...
	nftablesRules = append(nftablesRules, []string{
		...
		"        flags interval",
		"        elements = {",
		"            ::/96, # deprecated IPv4-compatible, covers ::1 loopback and :: unspecified",
		"            ff00::/8, # multicast",
		...
		fmt.Sprintf("        ct state new meta skuid >= %d meta l4proto udp ip daddr 127.53.53.53 udp dport 53 counter accept comment \"tenant UDP DNS\"", tenantUIDRangeBase),
		fmt.Sprintf("        ct state new meta skuid >= %d meta l4proto tcp ip daddr 127.53.53.53 tcp dport 53 counter accept comment \"tenant TCP DNS\"", tenantUIDRangeBase),
...
func (agt *agent) setupIpvsadm(lnc cdntypes.L4LBNodeConfig, l4lbConfPath string) error {
	...
	// -A -t <service-ipv4>:80 -s mh -b mh-fallback,mh-port
	// -a -t <service-ipv4>:80 -r <cache-node-ipv4>:80 -i -w 1 --tun-type ipip
	// -A -t <service-ipv4>:443 -s mh -b mh-fallback,mh-port
	// -a -t <service-ipv4>:443 -r <cache-node-ipv4>:443 -i -w 1 --tun-type ipip
	// -A -t [<service-ipv6>]:80 -s mh -b mh-fallback,mh-port
	// -a -t [<service-ipv6>]:80 -r [<cache-node-ipv6>]:80 -i -w 1 --tun-type ipip
	// -A -t [<service-ipv6>]:443 -s mh -b mh-fallback,mh-port
...
func (agt *agent) cleanUpService(orgPath string, orgID pgtype.UUID, serviceID pgtype.UUID) error {
	...
	if loadState == "loaded" {
		...
		stdout, stderr, err := agentutils.RunCommand("systemctl", "disable", systemdBaseName)
		if err != nil {
			agt.logger.Err(err).Str("stdout", stdout).Str("stderr", stderr).Str("pattern", systemdBaseName).Msg("systemctl disable failed")
			return err
		}

		stdout, stderr, err = agentutils.RunCommand("systemctl", "stop", systemdBaseName)
		if err != nil {
		...
		//stdout, stderr, err = agentutils.RunCommand("systemctl", "reset-failed", systemdBaseName)
		//if err != nil {
...
func (agt *agent) generateCacheFiles(cnc cdntypes.CacheNodeConfig) {
	...
	if len(allIPAddresses) > 0 {
		...
		if modified {
			...
			stdout, stderr, err := agentutils.RunCommand("networkctl", "reload")
			if err != nil {
...
func (agt *agent) systemctlShow(pattern string) (map[string]string, error) {
	...
	stdout, stderr, err := agentutils.RunCommand("systemctl", "show", pattern)
	if err != nil {
...
func (agt *agent) systemctlDaemonReload() error {
	...
	stdout, stderr, err := agentutils.RunCommand("systemctl", "daemon-reload")
	if err != nil {
...
func (agt *agent) enableUnitFile(name string) (bool, error) {
	...
	if unitFileState == "disabled" {
		...
		stdout, stderr, err := agentutils.RunCommand("systemctl", "enable", name)
		if err != nil {
		...
		if activeState != "active" || subState != "running" {
			...
			stdout, stderr, err := agentutils.RunCommand("systemctl", "start", name)
			if err != nil {
